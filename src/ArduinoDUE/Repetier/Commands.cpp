...
void Commands::printTemperatures(bool showRaw) {
    ...
    Com::printF(Com::tColon, pwm_pos[PWM_FAN_THERMO]);
#elif NUM_EXTRUDER == 1
    if(showRaw) {
...
void Commands::processMCode(GCode *com) {
    switch( com->M ) {
        case 19:  //  Create memory position on SDcard
          sd.saveCurrentPosition();
          break;
#if SDSUPPORT
        ...
        case 117: // M117 message to lcd
            ...
            break;
        case 118: // Print cabinet temerature to lcd
          tmpMessage = "T: " + String(tempController[3]->currentTemperatureC) + "/" + String(Printer::thermoMinTemp) + ":" + String(pwm_pos[PWM_FAN_THERMO]);
          tmpMessage.toCharArray(message, 20);
          UI_STATUS_UPD_RAM(message);
          break;
        case 119: // M119
            Commands::waitUntilEndOfAllMoves();
            Endstops::update();
...
#endif
#if FAN_THERMO_PIN > -1
        case 460: // Anteino: M460 S[wanted chamber temperature]
            Printer::resetFanThermoPID();
            Printer::thermoMinTemp = com->S;
            break;
#endif
        case 500: { // M500
#if EEPROM_MODE != 0
