...
#define EVENT_HEATER_DECOUPLED(id) {if(!Printer::errorDetected){Printer::errorDetected = true; char msg[128]; sprintf(msg, "Extruder %d is decoupled.", id); Serial.println(msg);}/*if(!Printer::errorDetected){Printer::errorDetected = true; sd.saveStopPrint(false);}*/}
// Gets called if a missing/shorted thermistor is detected.
#define EVENT_HEATER_DEFECT(id) {if(!Printer::errorDetected){Printer::errorDetected = true; char msg[128]; sprintf(msg, "Extruder %d is defect.", id); Serial.println(msg);}/*if(!Printer::errorDetected){Printer::errorDetected = true; sd.saveStopPrint(false);}*/}
