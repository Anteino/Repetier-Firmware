...
/*
 * Anteino 19-12-2017: We can't use PWM on the pin the case light is connected to. The timer used
 * to do PWM on this pin is already used for different functions by the firmware so we may not
 * interfere. You could connect a decent capacitor in parallel with the led strip and try to use
 * the timers in HALL.cpp. All you need to do is comment WRITE(CASE_LIGHTS_PIN,s) and uncomment
 * the line below. You can then use M355 S[0 - 255] to control the intensity.
 */
//  pwm_pos[PWM_CASE_LIGHT] = s;
    WRITE(CASE_LIGHTS_PIN,s); //  Comment this line if you decide to uncomment the above line
...
#if (DRIVE_SYSTEM == CARTESIAN || DRIVE_SYSTEM == GANTRY_FAKE)
